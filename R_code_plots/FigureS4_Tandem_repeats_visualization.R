...
UL1690_length <- setNames(read.table('cc1690_HiFi_chr_length.txt', sep="\t", fill=TRUE),c("Chr","Length"))
UL1690_length <- UL1690_length%>% mutate(order = n() - row_number() + 1) 
UL1690_gap <- setNames(read.table('cc1690_HiFi_gap.bed', sep="\t", fill=TRUE),c("Chr","Start","End"))
...
cc1690_gap <- setNames(read.table('cc1690_nanopore_gap.bed', sep="\t", fill=TRUE),c("Chr","Start","End"))
...
cc400_gap <- setNames(read.table('cw15_HiFi_gap.bed', sep="\t", fill=TRUE),c("Chr","Start","End"))
...
UL1690
ggsave(file="png/UL1690_repeats.png", UL1690, width=25, height=8)
...
cc5816
ggsave(file="png/cc5816_repeats.png", cc5816, width=25, height=8)
...
cc400
ggsave(file="png/cc400_repeats.png", cc400, width=25, height=8)
...
cc4532
ggsave(file="png/cc4532_repeats.png", cc4532, width=25, height=8)
...
cc1690
ggsave(file="png/cc1690_repeats.png", cc1690, width=25, height=8)
...
cc4532_all_TRF <- setNames(read.table('cc4532_v6.1.fa.2.7.7.80.10.50.2000.dat.bed', sep="\t", fill=TRUE),name_list)
cc4532_all_TRF_filtered <- cc4532_all_TRF %>% filter(Copies>50 & Period>50)
cc4532_all_TRF_filtered_contig27 <- cc4532_all_TRF_filtered %>% filter(Chr=="contig_27") %>% select(Chr, Start,End,Copies,Repeats)
write.table(cc4532_all_TRF_filtered_contig27, "extract_contig27.txt", sep = "\t", quote = FALSE, col.names = FALSE, row.names = FALSE)
