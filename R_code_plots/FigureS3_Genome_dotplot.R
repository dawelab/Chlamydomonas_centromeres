...
#ptions(error=traceback)

#arser <- OptionParser(usage = "%prog -i alignments.coords -o out [options]",option_list=option_list)
#pt = parse_args(parser)

# rm(list=ls())
setwd("/Users/mingyu/Desktop/cc1690_cc5816_default/")
#opt = list(input_filename="cc1690_HiFi_chr_to_cc5816.paf",
           #output_filename="cc1690_hifi_cc5816",
           #min_align = 1000,min_query_aln =2000000,
           #keep_ref=23, 
           #similarity=T, h_lines=T, interactive=F, plot_size=40,on_target = T, v=FALSE)

opt = list(input_filename="cw15_HiFi_chr_to_Dutcher.paf",
           output_filename="cw15_hifi_cc5816",
           min_align = 1000,min_query_aln =2000000,
           keep_ref=23, 
           similarity=T, h_lines=T, interactive=F, plot_size=40,on_target = T, v=FALSE)
...
if (opt$similarity) {
  gp = ggplot(alignments) +
    ...
    theme(
      ...
      axis.text.y = element_text(size =30,angle = 15,face = "bold"),
      axis.text.x = element_text(size =30,angle = 75,vjust = -.001,face = "bold"),
      plot.margin = margin(2,2,80,2)
    ) +
...
# gp
ggsave(filename = paste0(opt$output_filename, ".png"), width = 50, height = 30, units = "in", dpi = 300, limitsize = F)
